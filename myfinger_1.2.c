...
void InitList();				// 히스토리 초기화
void Add_history(hlist *tmp, char *data); // 히스토리 추가
hlist *Find(hlist *serch, char data);				// 히스토리 검색
void clearBuffer(void)						// 버퍼 초기화
...
void Add_history(hlist *tmp, char *data)
{
...
bool run(char* line, hlist *head, hlist *tail) 
{
	...
	Add_history(&head, line);	// 실행한 명령어 히스토리 리스트에 추가
...
int main() {
	...
	while (1)
	{
		...
		while (fgets(line, sizeof(line) - 1, stdin))
		{
			...
			else if(run(line, &head, &tail) == false)
				break;
		}
	}
	return 0;
